  ...
  'scriptrMap',
  {
    ...
    controller : function($scope, $rootElement, $location, $sce,
      ...
      self.$onInit = function() {
        ...
        $scope.$on("updateMapData", function(event, data) {
			loadMapData(data)
        });  
      ...
      var loadMapData =  function(apiParams) {
          	wsClient.onReady.then(function() {
              if(self.api) {
             	   	wsClient.call(self.api, apiParams).then(function(data, response) {
                      self.dynMarkers = [];  
      ...
      self.focusOnAsset = function(assetKey) {
        if(self.onSelectAsset && typeof self.onSelectAsset() == "function"){
            for(var i = 0; i < this.dynMarkers.length; i++){
                if(self.dynMarkers[i].assetKey == assetKey){
                    self.onSelectAsset()(self.dynMarkers[0]);
                    break;
                }
            }
        }
