        ...
        controller: function($scope,wsClient, httpClient) {
           ...
           //     this.filterName = (this.filterName) ? this.filterName : "Filter";
            ...
            self.onSelectItem = function(){
                self.total = 0;
                self.totalItems =  this.objects.length;
                for(var i = 0; i < this.objects.length > 0; i++){
                    self.total += parseInt(this.objects[i].price);
                }
                self.totalNewObjects = 0;
                for(var i = 0; i < this.newObjects.length > 0; i++){
                    self.totalNewObjects += parseInt(this.newObjects[i].price);
                }
            } 

            self.onEditItem = function(obj, data){
                console.log(obj);
                obj.comments = data;
            }
            ...
            self.onMarkAsEvent = function(){
                this.markedAs = true;
                if(typeof self.onMarkAs() == "function"){
                    self.onMarkAs()(self);
                }
            }
            ...
            this.sendOrder = function(objects){
                this.orderSent = true;
                if(typeof self.onSendOrder() == "function"){
                    self.onSendOrder()(objects, self);
                }
            }
